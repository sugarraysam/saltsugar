...
function exportPartitions() {
    if [ -b "${BOOTSTRAP_DISK}1" ]; then
        ...
        echo 2>&1 "Unsupported partition name, partition neither ends with 1 or p1."
        echo 2>&1 "Modify script to support partition listed here:"
        ls ${BOOTSTRAP_DISK}* 2>&1
        exit 1
    fi
...
function encryptDisk() {
    ...
    if [ -z "${BOOTSTRAP_LUKS}" ]; then
        echo 2>&1 "Please set BOOTSTRAP_LUKS."
        exit 1
    fi
...
function copyAndRunChroot() {
    ...
    cp ${PWD}/scripts/chroot.sh /mnt/tmp/chroot.sh
